...
- (id)init
{
	...
	if (self) {
		[self setOutputFile:[@"~/Desktop/Audio Recording.wav" stringByStandardizingPath]];
	}
...
- (void)awakeFromNib
{
    ...
    //effectAudioUnitComponentDescription.componentSubType = kAudioUnitSubType_MultiBandCompressor;
    //Une unité audio qui fournit 4 bandes de compression dynamique ou d'expansion.
   
    effectAudioUnitComponentDescription.componentSubType = kAudioUnitSubType_DynamicsProcessor;
     //Une unité audio qui fournit une compression dynamique ou expansion
    
   // effectAudioUnitComponentDescription.componentSubType = kAudioUnitSubType_NBandEQ;
    //Un égaliseur multibande avec type de filtre specifique pour chaque bande
...
- (IBAction)chooseOutputFile:(id)sender
{
	...
	[savePanel setAllowedFileTypes:[NSArray arrayWithObject:@"wav"]];
